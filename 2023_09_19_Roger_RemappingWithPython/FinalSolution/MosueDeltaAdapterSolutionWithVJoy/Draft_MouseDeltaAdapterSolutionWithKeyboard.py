...
# Warning this solution is for the keyboard version but
# It is not idea at all because we can't control the speed of the camera move as it is a mouse that is hardware controlled .
...
def cursror_display_state(isDisplay):
    if isDisplay:
        cursror_hide()
    else:
        cursror_display()
...
def action_changehand_keyboard():
    pyautogui.keyDown('ctrlright')
    pyautogui.keyUp('ctrlright')
    print(" action_changehand")
    
def action_courch_keyboard():
    pyautogui.keyDown('shift')
    pyautogui.keyUp('shift')
    print(" action_changehand")

def action_crouch_joystick():
...
def switch_running_state_onoff( runningState):
    ...
    if hide_mouse_in_playmode:
        cursror_display_state(not runningState)
    ...
    try:
        ...
        #    print("Spacebar is pressed!")
            ...
            switch_running_state_onoff(False)
                ...
                action_changehand_keyboard()
        elif key == keyboard.Key.backspace:
                action_courch_keyboard()
...
def ResetMouse():
    ...
    if hide_mouse_in_playmode:
        cursror_display_state(not running)
...
is_playerMoving=False
def set_player_moving(isMoving):
    global is_playerMoving
    changed = is_playerMoving != isMoving
    if(changed and isMoving):
        print ("Start moving")
        win32api.mouse_event(win32con.MOUSEEVENTF_LEFTDOWN,0,0)
        #pyautogui.mouseDown(button='left')
        
    if(changed and not isMoving):
        print ("Stop moving")
        win32api.mouse_event(win32con.MOUSEEVENTF_LEFTUP,0,0)
        #pyautogui.mouseUp(button='left')

    is_playerMoving=isMoving
